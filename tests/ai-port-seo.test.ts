...
describe("サイトマップ", () => {
  ...
  it("焼肉 千里 のURLが混ざっていない", () => {
    // 分離前は千里と1つの sitemap.xml を共有していました。
    // 単独リポジトリになったので、千里のページが残っていたら移植漏れです。
    for (const url of urls) {
      expect(url, url).not.toMatch(/\/(menu|access|takeout|store|reservation)$/);
    }
    expect(urls.some((url) => url.includes("yakiniku"))).toBe(false);
  });
