...
describe("URL / hreflang", () => {
  ...
  it("内部パスにベースパスを含めない（Next.js が付けるため）", () => {
    // 分離前は CRYPTO PORT と同居しており、ルートの `/{locale}/` が
    // 使用中だったため `/sports-port/{locale}/` に置いていました。
    //
    // 単独リポジトリになったので、内部パスはルート相対です。
    // GitHub Pages の `/sports-port` は Next.js の basePath が付けます。
    // ここで足すと `/sports-port/sports-port/ja` と二重になります。
    expect(base).toBe("");
    expect(href("ja", "/")).toBe("/ja");
    expect(href("ja", "/matches")).toBe("/ja/matches");
  });

  it("絶対URLに同じベースパスが二重に入らない", () => {
    const url = absoluteUrl("ja", "/matches");
    const matches = url.match(/\/sports-port(?=\/|$)/g) ?? [];
