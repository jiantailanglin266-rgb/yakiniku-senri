...
describe("URLの組み立て", () => {
  /*
    分離前は焼肉 千里 と同居していたため、内部パスが `/ai-port` から
    始まっていました。単独リポジトリになったので、内部パスはルート相対です。

    ⚠ 内部パスにベースパスを足さないでください。
      GitHub Pages の `/ai-port` は Next.js の basePath が付けます。
      ここで足すと `/ai-port/ai-port/tools` になります。
  */
  it("aiPortPath はルート相対（ベースパスを含めない）", () => {
    expect(aiPortPath("/")).toBe("/");
    expect(aiPortPath("/tools")).toBe("/tools");
  });

  it("aiPortUrl は絶対URLを返し、末尾スラッシュが重複しない", () => {
    // canonical・OGP・sitemap 用。こちらは Next.js を通らないため、
    // ベースパスを自分で含めます。
    expect(aiPortUrl("/")).toMatch(/^https?:\/\/[^/]+/);
    expect(aiPortUrl("/tools")).toMatch(/\/tools$/);
    expect(aiPortUrl("/tools/")).toMatch(/\/tools$/);
    expect(aiPortUrl("/tools")).not.toMatch(/\/tools\/$/);
  });

  it("絶対URLに同じベースパスが二重に入らない", () => {
    // portalBase で一度やらかした壊れ方です（/ai-port/ai-port/... になる）
    const url = aiPortUrl("/tools");
    const matches = url.match(/\/ai-port(?=\/|$)/g) ?? [];
    expect(matches.length, url).toBeLessThanOrEqual(1);
  });

  it("ナビゲーションのリンクがすべてルート相対", () => {
    for (const item of [...aiMainNav, ...aiTabNav]) {
      expect(item.href.startsWith("/"), item.label).toBe(true);
      expect(item.href.startsWith("/ai-port"), `${item.label} にベースパスが混ざっています`).toBe(
        false,
      );
    }
  });
