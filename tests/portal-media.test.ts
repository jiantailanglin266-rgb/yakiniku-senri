...
describe("ニュース／動画サイトマップ", () => {
  ...
  it("robots.txt が言語別のニュース／動画サイトマップを申告する", async () => {
    // 分離前は焼肉 千里 と同居しており、robots.txt は千里のドメインで配信されていました。
    // ポータルが別ドメインのときに存在しないURLを書かないよう、
    // 「オリジンが一致するときだけ申告する」という条件が付いていました。
    //
    // 単独リポジトリになり、robots.txt はポータル自身のドメインで配信されます。
    // サイトマップのURLも同じ origin から組み立てるため、
    // 別ドメインを指してしまう経路が構造上なくなりました。
    ...
    try {
      ...
      // 申告するURLは、すべて自分のオリジンであること
      for (const url of sitemaps) {
        expect(url, url).toMatch(/^https:\/\/example\.com\//);
      }
    } finally {
      vi.unstubAllEnvs();
      vi.resetModules();
