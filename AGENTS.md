...
CRYPTO PORT / AI PORT / CARD PORT / SPORTS PORT は、このリポジトリから分離しました。

- 移設先: `portals`（`portals-split` ブランチとして退避済み）
- 分離時のコミット: 「焼肉 千里の公式サイトから分離する」

このリポジトリは**焼肉 千里の公式サイトだけ**を配信します。
ポータルのコード・画像・ドキュメント・ワークフローを戻さないでください。

### 分離後も残している設計

| 項目                                   | 理由                                                              |
| -------------------------------------- | ----------------------------------------------------------------- |
| `(senri)` というルートグループ名と配置 | 名前を変えるとURLが変わり、既存の被リンク・ブックマークが切れます |
| 国旗（flag-icons / MIT）の表記         | 言語切り替えで使い続けるため、著作権表示の義務も残ります          |
