    ...
    >
      <div className="max-w-3xl space-y-8">
          ...
          <Panel className="p-5">
            <dl className="divide-line/30 divide-y">
              {[
                ...
                <div key={label} className="grid grid-cols-[8rem_1fr] gap-3 py-2.5">
                  ...
                  <dd className={value ? "text-mist text-[0.82rem]" : "text-amber text-[0.82rem]"}>
            ...
            <div className="border-line/50 mt-5 border-t pt-4">
              ...
              <ul className="space-y-3">
                {authors.map((author) => (
                  <li key={author.id}>
                    <p className="text-ink text-[0.84rem] font-medium">
                      {pick(author.name, locale)}
                      <span className="text-dim ms-2 text-[0.72rem]">
                        {pick(author.role, locale)}
                      </span>
                    </p>
                    <p className="text-mist mt-0.5 text-[0.76rem] leading-relaxed">
        ...
        {page.sections.map((section, index) => (
          <section key={index}>
            <h2 className="text-ink mb-3 text-[1.05rem] font-semibold">
              {pick(section.heading, locale)}
            </h2>
            <ul className="space-y-2.5">
              {pickList(section.body, locale).map((line) => (
                <li key={line} className="text-mist flex gap-2.5 text-[0.88rem] leading-[1.9]">
              ...
              <dl className="divide-line/30 divide-y">
                {scoreAxes.map((axis) => (
                  <div key={axis} className="px-5 py-3.5">
                    <dt className="text-cyan text-[0.84rem] font-medium">
                      {pick(axisLabels[axis], locale)}
                    </dt>
                    <dd className="text-mist mt-1 text-[0.8rem] leading-relaxed">
