        ...
        {diagnoses.map((diagnosis) => (
          <li key={diagnosis.id}>
            <Link href={routes.diagnosis(locale, diagnosis.slug)} className="block h-full">
              <Panel glow className="hover:border-cyan/40 h-full p-5 transition-colors">
                <Badge accent={diagnosis.accent}>{diagnosis.questions.length} Q</Badge>
                <h2 className="text-ink mt-3 text-[0.96rem] font-semibold">
                  {pick(diagnosis.title, locale)}
                </h2>
                <p className="text-mist mt-2 text-[0.79rem] leading-relaxed">
                  {pick(diagnosis.lead, locale)}
                </p>
                <p className="text-cyan mt-4 text-[0.78rem]">{dictionary.diagnosis.start} →</p>
              </Panel>
            </Link>
          </li>
        ))}
