        ...
        {simulators.map((simulator) => (
          <li key={simulator.id}>
            <Link href={routes.simulator(locale, simulator.slug)} className="block h-full">
              <Panel glow className="hover:border-emerald/40 h-full p-5 transition-colors">
                <Badge accent={simulator.accent}>SIM</Badge>
                <h2 className="text-ink mt-3 text-[0.94rem] font-semibold">
                  {pick(simulator.title, locale)}
                </h2>
                <p className="text-mist mt-2 text-[0.79rem] leading-relaxed">
                  {pick(simulator.lead, locale)}
                </p>
                <p className="numeric text-cyan bg-navy/60 border-line mt-3 rounded-lg border px-2.5 py-1.5 text-[0.7rem]">
