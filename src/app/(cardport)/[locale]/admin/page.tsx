            ...
            <ul className="divide-line/30 divide-y">
              {links.map((link) => (
                <li key={link.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                  <span className="text-mist text-[0.8rem]">{link.id}</span>
                  <span className="text-dim text-[0.72rem]">{link.program}</span>
                  <Badge
                    accent={
                      link.status === "active"
                        ? "emerald"
          ...
          <Panel className="overflow-hidden">
            <ul className="divide-line/30 divide-y">
              {expiring.map(({ campaign, expired }) => (
                ...
                >
                  <span className="text-mist line-clamp-1 flex-1 text-[0.78rem]">
                    {pick(campaign.title, locale)}
                  </span>
                  <span
                    className={cx(
                      "numeric shrink-0 text-[0.74rem]",
                      expired ? "text-danger" : "text-emerald",
                    )}
                  >
              ...
              {stale.map(({ card, age }) => (
                <li key={card.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                  <Link
                    ...
                    className="text-mist hover:text-cyan text-[0.8rem]"
                  >
                    {pick(card.name, locale)}
                  </Link>
                  <span className="numeric text-amber text-[0.74rem]">
