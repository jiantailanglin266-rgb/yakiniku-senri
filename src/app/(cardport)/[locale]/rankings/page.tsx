      ...
      notice={
        <div className="space-y-3">
          ...
          <Notice>
            ...
            <Link href={routes.policy(locale, "ranking-criteria")} className="text-cyan underline">
            ...
            return (
              <li key={category.id}>
                <Panel className="h-full p-4">
                  <h2 className="text-ink text-[0.9rem] font-semibold">
                    <Link
                  ...
                  <ol className="mt-3 space-y-1.5">
                    {top.map((entry) => (
                      <li key={entry.card.id} className="flex items-center gap-2 text-[0.78rem]">
                        <span className="numeric text-gold w-4 shrink-0">{entry.rank}</span>
                        <Link
                          href={routes.card(locale, entry.card.slug)}
                          className="text-mist hover:text-cyan flex-1 truncate"
                        >
                          {pick(entry.card.name, locale)}
                        </Link>
                        <span className="numeric text-dim shrink-0">{entry.score.toFixed(2)}</span>
                      </li>
                    ))}
                  </ol>
                </Panel>
