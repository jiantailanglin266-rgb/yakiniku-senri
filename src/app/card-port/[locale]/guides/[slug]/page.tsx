            ...
            <ol className="space-y-1.5">
              {guide.sections.map((section, index) => (
                <li key={index}>
                  <a
                    href={`#section-${index}`}
                    className="text-cp-mist hover:text-cp-cyan text-[0.8rem] transition-colors"
                  >
            ...
            <section key={index} id={`section-${index}`} className="mb-10 scroll-mt-24">
              ...
              <ul className="space-y-3">
                {pickList(section.body, locale).map((line) => (
                  <li key={line} className="text-cp-mist flex gap-2.5 text-[0.9rem] leading-[1.9]">
                    <span className="text-cp-cyan shrink-0">・</span>
                    {line}
                  </li>
                ))}
              </ul>
