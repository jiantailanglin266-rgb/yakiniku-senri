      ...
      <div className="grid gap-8 lg:grid-cols-[22rem_1fr]">
        ...
        <div className="space-y-5 lg:sticky lg:top-24 lg:self-start">
          ...
          <Panel className="p-4">
            ...
            <Link
              ...
              className="text-cp-dim hover:text-cp-cyan mt-3 inline-block text-[0.7rem] underline"
            >
        ...
        <div className="space-y-8">
          <section aria-labelledby="conclusion">
            ...
            <Panel className="p-5">
              <p className="text-cp-mist text-[0.9rem] leading-relaxed">
          ...
          {activeCampaign ? (
            <section aria-labelledby="campaign">
              ...
              <Panel className="border-cp-amber/30 p-5">
                <p className="text-cp-amber text-[0.92rem] font-semibold">
                  {pick(activeCampaign.title, locale)}
                </p>
                <p className="text-cp-dim mt-1 text-[0.74rem]">
                  {pick(activeCampaign.target, locale)} / 〜
                  {formatDate(activeCampaign.endsOn, locale)}
                </p>
                {/* 条件は必ず全文出します。畳んで隠すと利用条件を隠した表示になります */}
                <ul className="text-cp-mist mt-3 space-y-1.5 text-[0.8rem]">
          ...
          <section aria-labelledby="proscons" className="grid gap-4 sm:grid-cols-2">
            <Panel className="p-5">
              <h2 id="proscons" className="text-cp-emerald mb-3 text-[0.95rem] font-semibold">
                {dictionary.card.pros}
              </h2>
              <ul className="text-cp-mist space-y-2 text-[0.82rem]">
                {pickList(card.pros, locale).map((line) => (
                  <li key={line} className="flex gap-2">
                    <span className="text-cp-emerald">+</span>
                    {line}
                  </li>
                ))}
              </ul>
            ...
            <Panel className="p-5">
              <h2 className="text-cp-danger mb-3 text-[0.95rem] font-semibold">
                {dictionary.card.cons}
              </h2>
              <ul className="text-cp-mist space-y-2 text-[0.82rem]">
                {pickList(card.cons, locale).map((line) => (
                  <li key={line} className="flex gap-2">
                    <span className="text-cp-danger">−</span>
                    {line}
                  </li>
                ))}
              </ul>
              ...
              <dl className="divide-cp-line/30 divide-y">
                {[
                  ...
                  <div key={label} className="grid grid-cols-[10rem_1fr] gap-3 px-5 py-3">
                    ...
                    <dd className="text-cp-mist text-[0.82rem]">{value}</dd>
                  </div>
                ))}
              </dl>
            </Panel>
            ...
            <Panel className="p-5">
              <ul className="text-cp-mist space-y-2 text-[0.82rem]">
                {pickList(card.notes, locale).map((line) => (
                  <li key={line} className="flex gap-2">
                    <span className="text-cp-amber">!</span>
                    {line}
                  </li>
                ))}
              </ul>
          ...
          <section aria-labelledby="whofor" className="grid gap-4 sm:grid-cols-2">
            <Panel className="p-5">
              <h2 id="whofor" className="text-cp-cyan mb-3 text-[0.95rem] font-semibold">
                {dictionary.card.recommendedFor}
              </h2>
              <ul className="text-cp-mist space-y-1.5 text-[0.82rem]">
            ...
            <Panel className="p-5">
              <h2 className="text-cp-dim mb-3 text-[0.95rem] font-semibold">
                ...
                {relatedNews.map((article) => (
                  <li key={article.id}>
                    <Link
                      ...
                      className="glass hover:border-cp-cyan/40 block rounded-xl px-4 py-3 transition-colors"
                    >
                      <span className="text-cp-mist text-[0.82rem]">
                        {pick(article.title, locale)}
                      </span>
                      <span className="text-cp-dim numeric ms-2 text-[0.7rem]">
            ...
            {card.categories.map((categoryId) => {
              ...
              return (
                <Link
                  ...
                  className="border-cp-line text-cp-mist hover:border-cp-cyan/50 hover:text-cp-ink rounded-full border px-3 py-1 text-[0.72rem] transition-colors"
                >
