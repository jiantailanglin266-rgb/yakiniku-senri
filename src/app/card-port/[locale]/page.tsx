  ...
  return (
    <>
      ...
      <Section id="ranking" labelledBy="ranking-heading">
        ...
        <p className="text-cp-dim mt-5 text-[0.72rem]">
          <Link
            href={routes.policy(locale, "ranking-criteria")}
            className="hover:text-cp-cyan underline"
          >
            ...
            <Reveal as="li" key={diagnosis.id} delayIndex={index % 6}>
              <Link href={routes.diagnosis(locale, diagnosis.slug)} className="block h-full">
                <Panel glow className="hover:border-cp-cyan/40 h-full p-4 transition-colors">
                  ...
                  <p className="text-cp-mist mt-1.5 line-clamp-2 text-[0.75rem] leading-relaxed">
            ...
            <Reveal key={category.id} delayIndex={index}>
              <Link href={routes.cardCategory(locale, category.id)}>
                <Panel className="hover:border-cp-cyan/40 p-4 transition-colors">
                  <p className="text-cp-ink text-[0.86rem] font-semibold">
                    {pick(category.title, locale)}
                  </p>
                  <p className="text-cp-dim numeric mt-1 text-[0.7rem]">
      ...
      <Section id="compare" labelledBy="compare-heading">
        ...
        <Panel className="overflow-x-auto p-1">
          <table className="sticky-col w-full min-w-[42rem] border-collapse text-[0.78rem]">
            ...
            <thead>
              <tr className="border-cp-line/60 text-cp-dim border-b text-[0.7rem]">
            ...
            <tbody>
              {overall.map((entry) => (
                <tr key={entry.card.id} className="border-cp-line/30 border-b last:border-0">
                  <th scope="row" className="p-3 text-start font-normal">
                    <Link
                      href={routes.card(locale, entry.card.slug)}
                      className="text-cp-ink hover:text-cp-cyan"
                    >
                      {pick(entry.card.name, locale)}
                    </Link>
                  </th>
                  <td className="numeric text-cp-mist p-3 text-end">
                    {entry.card.annualFee === 0
                      ? dictionary.common.free
                      : entry.card.annualFee.toLocaleString("ja-JP")}
                  </td>
                  <td className="numeric text-cp-cyan p-3 text-end">{entry.card.baseRate}%</td>
                  <td className="numeric text-cp-mist p-3 text-end">{entry.card.maxRate}%</td>
                  <td className="numeric text-cp-mist p-3 text-end">
                    {entry.card.issueDays}
                    {dictionary.card.days}
                  </td>
                  <td className="numeric text-cp-gold p-3 text-end font-semibold">
          ...
          {simulators.slice(0, 5).map((simulator, index) => (
            <Reveal as="li" key={simulator.id} delayIndex={index}>
              <Link href={routes.simulator(locale, simulator.slug)} className="block h-full">
                <Panel className="hover:border-cp-emerald/40 h-full p-4 transition-colors">
                  <Badge accent={simulator.accent}>SIM</Badge>
                  <h3 className="text-cp-ink mt-2.5 text-[0.84rem] leading-snug font-semibold">
              ...
              {getNews(5).map((article, index) => (
                <li key={article.id}>
                  <Link
                    ...
                    className="glass hover:border-cp-cyan/40 flex items-start gap-3 rounded-xl px-4 py-3 transition-colors"
                  >
                    <span className="numeric text-cp-gold shrink-0 text-[0.95rem] font-bold">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <span className="text-cp-mist text-[0.8rem] leading-snug">
              ...
              {getVideos({ limit: 5 }).map((video, index) => (
                <li key={video.id}>
                  <Link
                    ...
                    className="glass hover:border-cp-magenta/40 flex items-start gap-3 rounded-xl px-4 py-3 transition-colors"
                  >
                    <span className="numeric text-cp-magenta shrink-0 text-[0.95rem] font-bold">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <span className="text-cp-mist text-[0.8rem] leading-snug">
