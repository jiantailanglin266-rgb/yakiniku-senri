    ...
    >
      ...
      <Panel className="mb-8 p-4">
        ...
        <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {scoreAxes.map((axis) => (
            <li key={axis} className="flex items-center gap-2 text-[0.76rem]">
              <span className="text-cp-dim w-28 shrink-0">{pick(axisLabels[axis], locale)}</span>
              <span className="bg-cp-slate/70 h-1.5 flex-1 overflow-hidden rounded-full">
                <span
                  className="from-cp-cyan to-cp-violet block h-full rounded-full bg-gradient-to-r"
                  style={{ width: `${weights[axis] * 100}%` }}
                />
              </span>
              <span className="numeric text-cp-mist w-10 shrink-0 text-end">
        ...
        <Link
          ...
          className="text-cp-cyan mt-3 inline-block text-[0.74rem] underline"
        >
