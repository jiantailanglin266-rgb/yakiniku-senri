 ...
 *   それ以外の素材（配布サイトのアイコンセットなど）は取得経路も
 *   クレジットの書き方も異なります。同じ型に押し込むと
 *   「API で確認した情報」と「人が書き写した情報」の区別が消えるため、分けています。
 *
 * ■ ここに書いてよいのは、実際にリポジトリへ取り込んだ素材だけです。
 *   将来使うかもしれない候補は書きません（掲載していない素材の
 *   クレジットを出すと、逆に事実と異なる表示になります）。
 */
import type { LocalizedText } from "@/sports/types";
...
export const thirdPartyAssets: ThirdPartyAsset[] = [
  {
    ...
    name: "flag-icons",
    usage: {
      ja: "言語切り替えに表示している国旗（42点）。/images/flags/ 以下に WebP へ変換して収録しています。",
      en: "The 42 flags shown in the language switcher, converted to WebP under /images/flags/.",
    },
