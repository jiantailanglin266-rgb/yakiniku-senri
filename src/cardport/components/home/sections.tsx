    ...
    <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {articles.map((article, index) => (
        <Reveal as="li" key={article.id} delayIndex={index % 6}>
          <NewsCard article={article} locale={locale} dictionary={dictionary} />
        </Reveal>
      ))}
    </ul>
  );
          ...
          <Link href={routes.video(locale, video.slug)} className="block h-full">
            ...
            >
              ...
              <div className="from-cp-violet/40 via-cp-magenta/25 to-cp-navy relative aspect-video bg-gradient-to-br">
                ...
                <span className="numeric absolute right-2 bottom-2 rounded bg-black/60 px-1.5 py-0.5 text-[0.66rem] text-white">
