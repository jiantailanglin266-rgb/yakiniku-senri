...
## このリポジトリには2つのサイトが入っています

| サイト                                    | URL                             | ドキュメント                                           |
| ----------------------------------------- | ------------------------------- | ------------------------------------------------------ |
| **焼肉 千里**（このREADME）               | `/`, `/menu`, `/news/...`       | 以下                                                   |
| **CARD PORT**（フィンテック比較ポータル） | `/ja/`, `/en/`, `/ja/cards/...` | [`docs/cardport/README.md`](./docs/cardport/README.md) |

Next.js の複数ルートレイアウト（route groups）で完全に分離しています。
ルートグループ名はURLに出ないため、**焼肉 千里のURLは従来どおり**です。
スタイルも別のCSSエントリに分かれており、互いのテーマ変数は混ざりません。

- 焼肉 千里のソース … `src/app/(senri)/**`, `src/components/**`, `src/data/**`, `src/lib/**`
- CARD PORT のソース … `src/app/(cardport)/**`, `src/cardport/**`
